...
class MPMLearning:
    ...
    def multi_training_re(self, x, y, model_para, test_size=0.3, train_num=100, n_jobs=4, random_state=None, multioutput="uniform_average"):
        ...
        pool = mp.Pool(n_jobs)
        tasks = [pool.apply_async(MPMLearning.repeat_training_re, args=(self, x, y, cv, test_size, train_num, model_para, k, repeat, multioutput)) for k, repeat in enumerate(process_num)]
        pool.close()
        pool.join()
        results = [task.get() for task in tasks]
        for result in results:
    ...
    def multi_training(self, x, y, model_para, test_size=0.3, train_num=100, n_jobs=4, random_state=None, average='binary', pos_label=None, labels=None):
        ...
        process_num = MPMLearning.process_rep_allocation(self, train_num, n_jobs)
        pool = mp.Pool(n_jobs)
        tasks = [pool.apply_async(MPMLearning.repeat_training, args=(self, x, y, cv, test_size, train_num, model_para, k, repeat, average, pos_label, labels)) for k, repeat in enumerate(process_num)]
        pool.close()
        pool.join()
        results = [task.get() for task in tasks]
        for result in results:
    ...
    ## learning curve
    def learning_curve_plot(self, x, y, model_para=None, cv=None, n_jobs=4, train_step=np.linspace(.05, 1., 20), verbose=0, plot=True, scoring=None):
        root = tk.Tk()
        root.withdraw()
        Folderpath = filedialog.askdirectory()
        train_step, train_scores, test_scores = learning_curve(model_para, x, y, cv=cv, n_jobs=n_jobs, train_sizes=train_step, verbose=verbose, scoring=scoring)
        ...
        if plot:
            ...
            plt.ylabel(u'score')
            plt.grid()
            plt.fill_between(train_step, train_scores_mean - train_scores_std, train_scores_mean + train_scores_std, alpha=0.2, color='#1E7DC1')
            plt.fill_between(train_step, test_scores_mean - test_scores_std, test_scores_mean + test_scores_std, alpha=0.2, color='#CC4D65')
